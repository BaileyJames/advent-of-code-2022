...
def performInstructions(instructions: list, crates: list):
    ...
    for x in range(len(instructions[0])):
        ...
        if(int(currentInstruction[0]) > 1):
            print("running")
           
            while(len(tempArr) < int(currentInstruction[0])):
                tempArr.append(newCrates[int(currentInstruction[1]) -1][0])

                newCrates[int(currentInstruction[1]) -1].remove(newCrates[int(currentInstruction[1]) -1][0])
            if (len(tempArr) >= int(currentInstruction[0]) ):
                print("TRUE", tempArr)
                tempArr.reverse()
                for k in range(len(tempArr)):
                    newCrates[int(currentInstruction[2]) -1].insert(0, tempArr[k])
        else:
